...
def synthesise_control_seq_by_upsampling(mpc_control_seq,controller_time_step,sim_time_sampling):
    ...
    return upsampled_ctrl_sequence , upsampled_ctrl_sequence.shape[1]



def synthesise_control_seq_by_downsampling(upsampled_ctrl_seq, sim_time_sampling, controller_time_step):
    """
    Downsamples a control sequence to match the controller time step.

    Args:
        upsampled_ctrl_seq (numpy.ndarray): The upsampled control sequence of shape (num_controls, upsampled_length).
        sim_time_sampling (float): The time step of the upsampled control sequence.
        controller_time_step (float): The desired time step for the downsampled control sequence.

    Returns:
        numpy.ndarray: The downsampled control sequence.
        int: The length of the downsampled control sequence.
    """
    num_controls, len_upsampled = upsampled_ctrl_seq.shape
    # Generate the time arrays for upsampled and downsampled sequences
    upsampled_time = np.arange(0, len_upsampled * sim_time_sampling, sim_time_sampling)  # Time for upsampled sequence
    downsampled_time = np.arange(0, upsampled_time[-1] + controller_time_step, controller_time_step)  # Time for downsampled sequence
    
    # Initialize the downsampled control sequence
    downsampled_ctrl_sequence = np.zeros((num_controls, len(downsampled_time)))
    
    # Interpolate the control values to the downsampled time grid
    for i in range(num_controls):
        downsampled_ctrl_sequence[i, :] = np.interp(downsampled_time, upsampled_time, upsampled_ctrl_seq[i, :])
    
    return downsampled_ctrl_sequence, downsampled_ctrl_sequence.shape[1]
...
class RobotHospitalBed(RobotObj):
    def __init__(self, vehicle_type: str,current_time:np.float32,control_seq_length:int,control_update_rate:np.float32, do_upsampling=False):
    ...
    def _create_control_seqs(self,control_seqs,time_seq_start ):
        if self.do_upsampling :
            control_seqs  , control_seqs_len = synthesise_control_seq_by_upsampling(mpc_control_seq=control_seqs,\
                controller_time_step=self.control_dt , sim_time_sampling= self.robot_control_obj._sampling_time)
        a_f = control_seqs[0,:]
        a_r = control_seqs[1,:]
        s_f = control_seqs[2,:]
        s_r = control_seqs[3,:]
    ...
    def _append_control_seqs(self,control_seqs:np.ndarray, future_start_time )->None:
        ...
        self.robot_control_obj.a_f = np.hstack((self.robot_control_obj.a_f,a_f))
        self.robot_control_obj.a_r = np.hstack((self.robot_control_obj.a_r,a_r))
        self.robot_control_obj.s_f = np.hstack((self.robot_control_obj.s_f,s_f))
        self.robot_control_obj.s_r = np.hstack((self.robot_control_obj.s_r,s_r))
    ...
    def get_control_seqs_array_at_future(self, control_until_time_t ):
        ...
        print(f'{self.bold_text}{self.green_color}control_seqs_array_at_future,time-seqs{ self.robot_control_obj.time_seqs}--print temp-{temp}{self.reset_color}{self.reset_text}')
    ...
    def do_forward_sim(self,feedback_time,current_time, predicted_input_delay,do_upsampling=False):        
        ...
        for i in range(control_array.shape[1]):
            state_array+=self._getXdot(control_array[:,i][...,np.newaxis] , state_array)*self.robot_control_obj._sampling_time
     
        if self.do_upsampling :
            u_expected_seqs,seq_len =   synthesise_control_seq_by_downsampling(upsampled_ctrl_seq=u_expected_seqs, sim_time_sampling=self.robot_control_obj._sampling_time, controller_time_step=self.control_dt)
